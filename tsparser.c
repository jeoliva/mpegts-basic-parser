...
void parsePES(TSStream *stream, ABitReader *bitReader)
{
		...
		&& stream_id != 0xf8)   // H.222.1 type E
	{
        ...
        if (PTS_DTS_flags == 2 || PTS_DTS_flags == 3) 
		{
			...
			PTS = ((uint64_t)getBits(bitReader, 3)) << 30;
			skipBits(bitReader, 1);
            PTS |= ((uint64_t)getBits(bitReader, 15)) << 15;
            skipBits(bitReader, 1);
            ...
            if (PTS_DTS_flags == 3) 
			{
                ...
                DTS = ((uint64_t)getBits(bitReader, 3)) << 30;
                skipBits(bitReader, 1);
                DTS |= ((uint64_t)getBits(bitReader, 15)) << 15;
                skipBits(bitReader, 1);
                DTS |= getBits(bitReader, 15);
                skipBits(bitReader, 1);
        ...
        if (ESCR_flag) 
		{
            ...
            uint64_t ESCR = ((uint64_t)getBits(bitReader, 3)) << 30;
            skipBits(bitReader, 1);
            ESCR |= ((uint64_t)getBits(bitReader, 15)) << 15;
            skipBits(bitReader, 1);
            ESCR |= getBits(bitReader, 15);
